    ...
    <div style="max-width: 600px; margin: 10px auto;">
        ...
        <div v-loading="loading" style="margin-top: 25px;" v-if="active === 1">
            <el-form label-width="auto">
                ...
                <el-form-item>
                    <el-button type="primary" @click="handleBalance">修改余额</el-button>
                </el-form-item>
            </el-form>
        </div>
...
async function nextStep() {
    ...
    if (res.data.code == 0) {
        ...
        formData.value.balance = res.data.data.balance / 100;
        active.value++
    } else {
...
async function handleBalance() {
    ...
    try {
        loading.value = true;
        const res = await paxios.post("/manage/editUserBalance", { id: formData.value.userid, amount: amount, remark: remark, type: formData.value.type, businessNo: formData.value.businessNo });
        if (res.data.code != 0) {
            ElMessage.error(res.data.msg);
